...
export default function AgentPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Agent 问答"
        description="基于最近一次匹配分析结果，继续生成简历改写、面试题、关键词缺口和学习计划。"
      />
      ...
      <section className="mt-6 rounded-3xl bg-white p-6 shadow-sm">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">
              注意：当前后端 Agent 接口默认基于当前用户最近一次匹配结果工作。
              如果提示没有分析结果，请先去“匹配分析”页面跑一次分析。
            </p>
          </div>
        ...
        <div className="mt-5 rounded-3xl border border-slate-200 bg-slate-50 p-3">
          <textarea
            ...
            className="h-32 w-full resize-none bg-transparent p-3 text-sm leading-6 outline-none placeholder:text-slate-400"
              ...
              <div className="mb-3 flex items-center justify-between gap-3">
                ...
                {finalAnswer && (
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-full bg-white/10 px-3 py-1 text-xs text-white/70 hover:bg-white/20"
                  >
